...
class TestSalaryCalculator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            self.driver.find_element_by_class_name(self.private_policy_css_class_name).click()
    ...
    def test_value_text_validation(self):
        ...
        value_text = self.driver.find_element_by_id(self.value_text_id)
        value_text.click()
        value_text.send_keys("qwerty")
        evaluate = self.move_to_element(self.evaluate_button_id)
        evaluate.click()
        try:
            validation_error_elem = self.driver.find_element_by_xpath(self.validation_error_xpath)
    ...
    def test_net_salary_calculation(self):
        ...
        combobox = self.driver.find_element_by_id(self.gross_net_combobox_id)
        combobox.click()
        i = 1
        while i <= 1:
        ...
        net_value_number = self.driver.find_element_by_xpath(self.net_value_number_xpath)
        self.assertEqual(net_value_number.text, "5 500,00 PLN")
        net_value_text = self.driver.find_element_by_xpath(self.net_value_text_xpath)
        self.assertEqual(net_value_text.text, "kwota netto")
        self.close_advertisement()
        gross_value_number = self.driver.find_element_by_xpath(self.gross_value_number_xpath)
        self.assertEqual(gross_value_number.text, "7 797,72 PLN")
        gross_value_text = self.driver.find_element_by_xpath(self.gross_value_text_xpath)
    ...
    def close_advertisement(self):
        ...
        """
        try:
            advert_button = self.driver.find_element_by_xpath(self.advertisement_xpath)
            advert_button.click()
    ...
    def close_company_private_selection(self):
        ...
        """
        try:
            advert_button = self.driver.find_element_by_xpath(self.company_private_selection_class_name_xpath)
            advert_button.click()
    ...
    def move_to_element(self, element_id):
        ...
        actions.move_to_element(self.driver.find_element_by_id(element_id)).perform()
