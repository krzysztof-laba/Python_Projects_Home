...
def initialization():
	...
	# website.set_window_rect(0,0,1047,1173)
...
def check_if_page_exists():
	...
	try:
		...
		print("** DRUGIE sprawdzenie czy strona jest wyświetlona. **")
		# Checking after time elapsed:
		time.sleep(randint(5, 7))
		# Refresh before checking page.
		refresh_page()
		try:
			alexander = website.find_element_by_class_name('me')
			print("Strona działa. Aleander82 widoczny.")
			return alexander.is_enabled()
		except NoSuchElementException:
			print("Brak strony BW. Alexander NIE widoczny.")

			print("*** TRZECIE sprawdzenie czy strona jest wyświetlona. ***")
			# Checking after time elapsed:
			...
			try:
				...
				print("*** * CZWARTE sprawdzenie czy strona jest widoczna. * ***")
				# Checking after time elapsed:
				time.sleep(randint(3 * 60, 5 * 60))
				# Refresh before checking page.
				refresh_page()
				try:
					alexander = website.find_element_by_class_name('me')
					print("Strona działa po przerwie technicznej. Aleander82 widoczny.")
					return alexander.is_enabled()
				except NoSuchElementException:
					print("Brak strony BW. Alexander NIE widoczny.")

					print("*** ** P I Ą T E ostatnie sprawdzenie czy strona jest widoczna. ** ****")
					# Checking after time elapsed:
					time.sleep(randint(10 * 60, 15 * 60))
					# Refresh before checking page.
					refresh_page()
					try:
						alexander = website.find_element_by_class_name('me')
						print("Strona działa po przerwie technicznej. Aleander82 ponownie widoczny.")
						return alexander.is_enabled()
					except NoSuchElementException:
...
initialization()
time.sleep(0)
# Dress equipment to make expeditions (Yes/No).
if ubrac_wyprawkowy is True:
	grab_exp_eq()
else:
	print("Posiadasz ubrany wyprawkowy. Ustawione na:", ubrac_wyprawkowy)

# Run expeditions one by one.
print('*********	START EKSPEDYCJE	*********')
for exp in range(int(ilosc_wypraw)):
	if check_amount_of_expeditions() != '0':
		# Check amount of money.
		check_amount_of_money()
		# Start expeditions.
		if check_button_exp_exists() is True:
			# Start expedition.
			expeditions(exp)
			print("Ekspedycje wykonane.")
		else:
			print("Klawisz 'Rozpocznij Wyprawę' nie pojawił się przez ponad 10 minut...!!!")
			print("Zakończ wyprawy z powodu niedostępności strony")
			break
	else:
		print("Przerwano wykonywanie wypraw z powodu braku wypraw.")
		break
print('*********	STOP EKSPEDYCJE	*********')


# After expeditions change equipment on combat items.
choose_combat_eq()
# Return the expedition equipment.
return_exp_eq()
# Logout, close web browser and shutdown PC.
logout_and_close_web_browser()
